...
function default_1(socket, rooms) {
    ...
    const quizShowQuestion = () => {
        ...
        // verifico que se encuentre la sala
        if (roomFound && socket.data.role === "moderator") {
            const currentQuestion = roomFound.questions[roomFound.currentQuestion];
            // emito el evento que se ha iniciado el juego a todos los jugadores
            roomFound.players.forEach((p) => {
                socket
                    .to(p.socketId)
     ...
     */
    const sendAnswerPlayer = (index, countown) => {
        const roomFound = rooms.find((r) => r.code == socket.data.code);
        // verifico que se encuentre la sala
    ...
    const getRankingModerator = () => {
        ...
        if (roomFound && socket.data.role === "moderator") {
            ...
            roomFound.players.sort((a, b) => a.score < b.score ? 1 : a.score > b.score ? -1 : 0);
            socket.emit("quiz:ranking-moderator", roomFound.players);
        }
    };
    /**
    ...
    const nextQuestion = () => {
        ...
        if (roomFound && socket.data.role === "moderator") {
            ...
            if (roomFound.hasNext) {
                ...
                socket.emit("quiz:next-question", roomFound.hasNext, roomFound.currentQuestion);
                roomFound.players.forEach((p) => {
